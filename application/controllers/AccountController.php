...
class AccountController extends Controller {
	...
	public function loginAction() {
		if (!empty($_POST)) {
			if (isset($_SESSION['account'])) {
				$this->view->redirect('account/personal');
			}
			if (!$this->model->loginValidate($_POST)) {
				$this->view->message('error', $this->model->error);
			}
			elseif (!$this->model->checkData($_POST['phone'])) {
				$this->view->message('error', 'Логин или пароль указан неверно');
			}
			$this->model->login($_POST['phone']);
			$this->view->location('account/personal');
		}
	...
	public function personalAction(){
		$data = $this->model->userInfo($_SESSION['account']['id']);
		$params = [
			'name' => $data['name'],
			'phone' => $data['login'],
			'email' => $data['email'],
	...
	public function activeordersAction(){
		$this->view->render('Активные заказы');
	}
